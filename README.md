# Oracle_OCI-ShellScripts

#Multipart upload

https://github.com/oracle/oci-python-sdk/blob/master/examples/multipart_object_upload.py

# example taken from
http://www.dominicgiles.com/blog/files/edc48236165eb010807ef6e8262a1711-165.html

# this link contain lots of oracle cloud examples for python
https://github.com/oracle/oci-python-sdk/tree/master/examples


# Link how to generate PEM file
https://docs.cloud.oracle.com/iaas/Content/API/Concepts/apisigningkey.htm#How

# Shell script for multipart upload
https://github.com/oracle/oci-cli/blob/master/scripts/large_file_test_script.sh
https://github.com/oracle/oci-cli/tree/master/scripts/examples
